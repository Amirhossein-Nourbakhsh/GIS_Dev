...
class Oracle:
    ...
    def call_procedure(self,procedure_name,orderID):
        self.connect_to_oracle()
        cursor = self.cursor
        try:
            outValue = 'Y'
            ...
            if func !=[] and len(func)==1:
                try:
                    output = cursor.callproc(func[0],[outValue,str(orderID),])
                except ValueError:
                    output = cursor.callproc(func[0],[outValue,str(orderID),])
                except AttributeError:
                    output = cursor.callproc(func[0],[outValue,str(orderID),])
...
def addERISpoint(pointInfo,mxd,output_folder,out_points=r'points.shp'):
    ...
    erisIDs_4points = dict((_.get('DATASOURCE_POINTS')[0].get('ERIS_DATA_ID'),[('m%sc'%(_.get("MAP_KEY_LOC"))) if _.get("MAP_KEY_NO_TOT")==1 else ('m%sc(%s)'%(_.get("MAP_KEY_LOC"), _.get("MAP_KEY_NO_TOT"))) ,int('%s'%(1 if round(_.get("ELEVATION_DIFF"),2)>0.0 else 0 if round(_.get("ELEVATION_DIFF"),2)==0.0 else -1 if round(_.get("ELEVATION_DIFF"),2)<0.0 else 100))]) for _ in pointInfo)
...
def exportMultipage(mxd,output_folder,map_name,UTMzone,grid_size,erisPointLayer,buffer_dict,buffer_sizes_list,unit_code, buffer_name=r"buffer_%s.shp"):
    ...
    ddMMDDP.refresh()
    ddMMDDP.exportToPDF(os.path.join(output_folder,map_name), "ALL",resolution=200,layers_attributes='LAYERS_ONLY',georef_info=False)
    mxd.mxd.saveACopy(os.path.join(output_folder,"mxdMM.mxd"))
    return [temp,os.path.join(output_folder,map_name+".pdf")]
...
def exportViewerTable(ImagePath,FileName):
    ...
    delete_query = "delete from overlay_image_info where order_id = '%s' and type = '%s' and filename = '%s'"%(orderID,metaitem['type'],FileName)
    insert_query = "insert into overlay_image_info values (%s, %s, %s, %.5f, %.5f, %.5f, %.5f, %s, '', '')" % (str(orderID), orderInfo['ORDER_NUM'], "'" + metaitem['type']+"'", metaitem['lat_sw'], metaitem['long_sw'], metaitem['lat_ne'], metaitem['long_ne'],"'"+metaitem['imagename']+"'" )
    image_info = Oracle('prod').insert_overlay(delete_query,insert_query)

if __name__ == '__main__':
    try:
        ##  INPUT ##################
        orderID = '896857'#arcpy.GetParameterAsText(0).strip()#'736799'#
        ...
        scratch = r'C:\FIM'#arcpy.env.scratchFolder#r"E:\CC\luan\test1"#
        ...
        needTopo= Oracle('prod').call_function('printTopo',orderID)
        xplorerflag= Oracle('prod').call_procedure('xplorerflag',orderID)[0]
