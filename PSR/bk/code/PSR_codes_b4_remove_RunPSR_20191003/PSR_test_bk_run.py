...
########################### LOCAL ##################################
    OrderIDText = '427550'
    scratchfolder = r"E:\GISData_testing\temp\test1"  # for regular .shp etc.
...
######################################################################
    ...
    if UTMvalue[0]=='0':
        UTMvalue=' '+UTMvalue[1:]
    out_coordinate_system = arcpy.SpatialReference('WGS 1984 UTM Zone %sN'%UTMvalue)#os.path.join(connectionPath+'/', r"projections/NAD1983/NAD1983UTMZone"+UTMvalue+"N.prj")
...
##        }
    ...
    orderCentreSHP = arcpy.inhouseElevation_ERIS(orderCentreSHP).getOutput(0)
    Call_Google = ''
    ...
    if int(arcpy.GetCount_management(os.path.join(wells_merge)).getOutput(0)) != 0:
        ...
        arcpy.ImportToolbox(PSR_config.tbx)
        wells_sja = arcpy.inhouseElevation_ERIS(wells_sja).getOutput(0)
...
##    shutil.copy(reportcheck_path + r'\\'+OrderNumText+'_US_PSR.pdf', scratchfolder)  # occasionally get permission denied issue here when running locally
    arcpy.SetParameterAsText(1, r'success')#os.path.join(scratchfolder, OrderNumText+'_US_PSR.pdf'))
