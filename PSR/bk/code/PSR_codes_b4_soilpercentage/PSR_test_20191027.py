...
def getElevation(dataset,fields):
    ...
    inhouse_esri_geocoder = r"http://cabcvan1gis006/arcgis/rest/services/GPtools/pntElevation2/GPServer/pntElevation2/execute?env%3AoutSR=&env%3AprocessSR=&returnZ=false&returnM=false&f=pjson"
...
########################### LOCAL ##################################
    OrderIDText = '778013'
    scratchfolder = r"E:\CC\luan\test1\test11\new778013"  # for regular .shp etc.
...
##        }
    ...
    if (int(arcpy.GetCount_management('soillayer').getOutput(0)) == 0):   # no soil polygons selected
        ...
        try:
            ...
            for mapunit in reportdata:
                ...
                query = cur.callproc('eris_psr.InsertFlexRep', (OrderIDText, erisid, '9334', 2, 'S1', 1, 'Map Unit ' + mapunit['Musym'], ''))
                query = cur.callproc('eris_psr.InsertFlexRep', (OrderIDText, erisid, '9334', 2, 'N', 2, 'Map Unit Name:', mapunit['Map Unit Name']))
                if (len(mapunit) < 5):    #for Water, Urbanland and Gravel Pits
    ...
    orderCentreSHP = getElevation(orderCentreSHP,["Lon_X","Lat_Y","Id"])#arcpy.inhouseElevation_ERIS(orderCentreSHP).getOutput(0)
    Call_Google = ''
...
#                arcpy.SaveToLayerFile_management(r"wetlandclipNY_lyr",os.path.join(scratchfolder,"wetland.lyr"))
...
##    shutil.copy(reportcheck_path + r'\\'+OrderNumText+'_US_PSR.pdf', scratchfolder)  # occasionally get permission denied issue here when running locally
    arcpy.SetParameterAsText(1, os.path.join(scratchfolder,r'success'))# OrderNumText+'_US_PSR.pdf'))
