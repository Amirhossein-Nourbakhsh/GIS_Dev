...
for OrderIDText in templist:
    ...
    scratchfolder =os.path.join(r"\\cabcvan1gis006\GISData_testing\test1\psr3",OrderIDText)
    if not os.path.exists(scratchfolder):
        os.mkdir(scratchfolder)   # for regular .shp etc.
        arcpy.CreateFileGDB_management(scratchfolder,r"scratch.gdb")   # for tables to make Querytable
    scratch = os.path.join(scratchfolder,r"scratch.gdb")
    try:
        ##############
        #parameters to change for deployment
        connectionString = PSR_config.connectionString#'ERIS_GIS/gis295@GMTESTC.glaciermedia.inc'
...
##    ##    gc.collect()
    ...
    ########################### LOCAL ##################################
        # OrderIDText = '826721'
        # scratchfolder =r"\\cabcvan1gis006\GISData_testing\test1\psr3"   # for regular .shp etc.
    ...
    ##    data_topo = PSR_config.data_topo#"E:\GISData\Topo_USA\masterfile\Cell_PolygonAll.shp"
        ...
        eris_wells = r"\\cabcvan1gis006\GISData\PSR\python\mxd\ErisWellSites.lyr"#eris_wells = PSR_config.eris_wells#"E:\GISData\PSR\python\mxd\ErisWellSites.lyr"   #which contains water, oil/gas wells etc.
    ...
    ##    datalyr_wetlandNY = PSR_config.datalyr_wetlandNY
        ...
        orderGeometry= r"\\cabcvan1gis006\GISData_testing\test1\psr3\shp.shp"#os.path.join(scratchfolder,"orderGeometry.shp")
...
#        arcpy.AddField_management(orderGeometry, "UTM", "TEXT", "", "", "1500", "", "NULLABLE", "NON_REQUIRED", "")
        ...
        arcpy.SelectLayerByLocation_management(masterLayer_topo,'intersect',orderGeometryPR)
        ...
        if(int((arcpy.GetCount_management(masterLayer_topo).getOutput(0))) ==0):
            ...
            for item in infomatrix[:500]:
                ...
                if os.path.exists(os.path.join(tifdir_topo,tifname+ "_t.tif")):
                    ...
                    if not os.path.exists(os.path.join(scratchfolder,newtifname+'.tif')):
