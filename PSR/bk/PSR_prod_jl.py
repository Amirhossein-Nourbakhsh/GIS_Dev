...
########################### LOCAL ##################################
    OrderIDText = '881534'
    scratchfolder = r"C:\Users\JLoucks\Documents\JL\psr2"  # for regular .shp etc.
    #scratchfolder = r"W:\Data Analysts\Alison\_GIS\PSR_SCRATCHY\20200417117"
...
#        del in_row
    ...
    if (int(arcpy.GetCount_management(os.path.join(scratchfolder,"summary1_geol.dbf")).getOutput(0))== 0):
...
##        masterlyr_demCA = r"\\Cabcvan1fpr009\US_DEM\Canada_DEM_edited.shp"
    ...
    ########### Copy to non-instant (local code only remove for deployment!!!!!!!! #########

    noninstantfold = r'\\cabcvan1obi002\ErisData\Reports\prod\noninstant_reports\PSRmaps'
    if not os.path.exists(os.path.join(noninstantfold,OrderNumText)):
        os.mkdir(os.path.join(noninstantfold,OrderNumText))
    for jpegmap in os.listdir(scratchfolder):
        if jpegmap.endswith('.jpg'):
            shutil.copyfile(os.path.join(scratchfolder,jpegmap),os.path.join(r'\\cabcvan1obi002\ErisData\Reports\prod\noninstant_reports\PSRmaps',OrderNumText,jpegmap))
            print 'copying '+jpegmap
...
print "Final FIM report directory: " + (str(reportcheck_path + "\\" + OrderNumText + "_US_PSR.pdf"))
print "_____DONE"