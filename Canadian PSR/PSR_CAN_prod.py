...
# ------------------------------------------------------------------------------------------------------------------------
    ...
    OrderIDText ='1843242'
    OrderNumText = r"22112300005"
    scratchfolder = r'C:\Amir\scratch'
    if not os.path.exists(scratchfolder):
        os.mkdir(scratchfolder)
    arcpy.CreateFileGDB_management(scratchfolder, "scratch.gdb")
    scratch = os.path.join(scratchfolder,"scratch.gdb")   # for table to make Querytable
...
# OUTPUT ------------------------------------------------------------------------------------------------------------------------
    ...
    print("PSR_CAN_config.BedrockList" + str(PSR_CAN_config.BedrockList))
    print("PSR_CAN_config.SurficialList" + str(PSR_CAN_config.SurficialList))
    bufferDist_geol = '5 KILOMETERS'
...
# 4 GEOLOGY REPORT ------------------------------------------------------------------------------------------------------------------------
    ...
    if 'bufferDist_geol' in locals():
        ...
        if (int((arcpy.GetCount_management(masterLayer_bedrock).getOutput(0))) !=0):
            ...
            print("multipage_geology=" +str(multipage_geology))
            if multipage_geology == False:
                print("in the multipage geology false")
                # df.scale = 5000
                    ...
                    for in_row in in_rows:
                        for blist in bedrockclip_list:
                            ...
                            if tempID !="None" and tempID not in reportingList:
                                ...
                                print("************bedrock_fields.values()) =" + str(bedrock_fields.values()))
                                print("************bedrock_fields.values()[0] =" + str(bedrock_fields.values()[0]))

                                print("************bedrock_fields.keys() =" + str(bedrock_fields.keys()))
                                print("************bedrock_fields.keys()[0] =" + str(bedrock_fields.keys()[0]))

                                for key in bedrock_fields.keys()[1:]:
                                    print("************bedrock_fields[key] =" + str(bedrock_fields[key]))
                                    text_in_row = unicodedata.normalize('NFKD', in_row.getValue(key)).encode('ASCII', 'ignore').decode("ISO-8859-1")
                                    print("************in_row.getValue(key) =" + text_in_row)
                                    ...
                                    query = cur.callproc('eris_psr.InsertFlexRep', (OrderIDText, erisid, blist[1], 2, 'N',i, bedrock_fields[key]+": ", text_in_row ))
                                    i = i +1
                                    print query
