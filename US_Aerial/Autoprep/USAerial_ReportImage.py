    ...
    for image_year in imagedict.keys():
        ...
        for image in imagedict[image_year]:
            ...
            if FactoryCode == '':
                ...
                sr = arcpy.SpatialReference(int(FactoryCode))
            fin_image_name = os.path.join(job_fin,image_year + '_' + image_source + '_' +str(image_per_year) + '_' +image_comment + '.tif')
            ...
            centerlong = round(extent.XMin + (extent.XMax - extent.XMin)/2, 7)
            centerlat = round(extent.YMin + (extent.YMax - extent.YMin)/2, 7)
            set_imagedetail(extent,centerlat,centerlong,image_year + '_' + image_source + '_' +str(image_per_year) + '_' +image_comment + '.tif')
