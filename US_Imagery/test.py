...
def get_Footprint(inputRaster,FID):
    try:
        ...
        resampleRaster = os.path.join(ws,'resampleRaster' + FID + '.tif')
        bin_Raster = os.path.join(ws,'bin_Raster' + FID +  '.tif')
        polygon_with_holes= os.path.join(tmpGDB,'polygon_with_holes' + FID)
        ...
        expression = 'Con(' + '"' + 'resampleRaster' + FID + '.tif' + '"' + ' >= 10 , 1)'
        ...
        footprint_FC = arcpy.CreateFeatureclass_management(tmpGDB,"footprint_FC" + FID, "POLYGON", "", "DISABLED", "DISABLED", inputSR)
...
def apply_process(inputParam):
      ...
      arcpy.AddMessage('-------------------------------------------------------------------------------------------------')
      arcpy.AddMessage('Start FID: ' + FID + ' - processing Dataset: ' + tabfile_Path)
      
      fileName = get_ImageName(tabfile_Path)
      if len(fileName) > 0:
            ext = '.' + fileName.split('.')[1]
            image_Path = tabfile_Path.replace('.TAB',ext)
            ## start creating seamless map 
            metaData = get_Image_Metadata(image_Path,ext,FID)
            footprint_Polygon = get_Footprint(image_Path,FID)
      ...
      arcpy.AddMessage(('End FID: ' + str(inputParam[0]) + ' - processed Dataset. Duration:', round(endDataset - startDataset,4)))
...
if __name__ == '__main__':
    ...
    params =[]
    expression = "FID >= 205 AND FID <= 250"
#     expression = "FID = 35"
