...
class Order(object):
    ...
    def get_order(self,input_id):
        try:
            ...
            sql_statment = "select ord.order_num, ord.address1, ord.city, ord.provstate , ord.site_name, cus.customer_id, cus.company_id, comp.company_desc, ord.pobox, ord.postal_code from orders ord, customer cus, company comp  where ord.customer_id = cus.customer_id and cus.company_id = comp.company_id and order_id = " + str(input_id)
            ...
            if row is not None:
                ...
                sql_statment = sql_statment = "select ord.order_id, ord.address1, ord.city, ord.provstate, ord.site_name, cus.customer_id, cus.company_id, comp.company_desc, ord.pobox, ord.postal_code from orders ord, customer cus, company comp  where ord.customer_id = cus.customer_id and cus.company_id = comp.company_id and order_num = '" + str(input_id) + "'"
                cursor.execute(sql_statment)
