...
if __name__ == '__main__':
    ...
    order_obj = models.Order().get_order(str(21021600002))
    print("Order: " + order_obj.id + ", " + order_obj.number)

    print(tf.logger(cfg.logfile))
    ...
    try:
        ...
        try:
            ...
            coverInfotext = json.loads(cur.callfunc('eris_gis.getCoverPageInfo', str, (str(order_obj.id),)))
            ...
            cur.execute("select geometry_type, geometry, radius_type  from eris_order_geometry where order_id =" + order_obj.id)
            t = cur.fetchone()
            OrderType = str(t[0])
        ...
        try:
            ...
            cur.execute("select decode(c.company_id, 40385, 'Y', 'N') is_nova from orders o, customer c where o.customer_id = c.customer_id and o.order_id=" + str(order_obj.id))
            t = cur.fetchone()
            is_nova = t[0]
        ...
        try:
            ...
            cur.execute("select customer_id from orders where order_id = '"+ order_obj.id+"'")
            a = cur.fetchone()
            CustIDText = str(a[0])
        ...
        try:
            con = cx_Oracle.connect(cfg.connectionString)
            cur = con.cursor()
            newlogofile = cur.callfunc('ERIS_CUSTOMER.IsCustomLogo', str, (str(order_obj.id),))
        ...
        try:
            ...
            is_aei = cur.callfunc('ERIS_CUSTOMER.IsProductChron', str, (str(order_obj.id),))
        finally:
            cur.close()
            con.close()

        copydirs = []    # will keep directories to be zipped
        deliverfolder = order_obj.number
        pdfreport = order_obj.number+"_US_Topo.pdf"

        srGCS83 = arcpy.SpatialReference(os.path.join(cfg.prjpath, "GCSNorthAmerican1983.prj"))
        srWGS84 = arcpy.SpatialReference(os.path.join(cfg.prjpath, "WGS1984.prj"))
        ...
        if(int((arcpy.GetCount_management(masterLayer).getOutput(0))) ==0):
            ...
            (dict7575,dict7575_s) = tf.reorgByYear(maps7575)  # {1975: geopdf.pdf, 1973: ...}
            (dict1515,dict1515_s) = tf.reorgByYear(maps1515)
            (dict3060,dict3060_s) = tf.reorgByYear(maps3060)
            (dict12,dict12_s) = tf.reorgByYear(maps12)
            ...
            if is_aei == 'Y':
                ...
                tf.createPDF("7.5",dict7575,dict7575_s,"map_" + order_obj.number + "_75.pdf")
                tf.createPDF("15",dict1515,dict1515_s,"map_" + order_obj.number + "_15.pdf")
    ...
    # Save Summary #######################################################################################################
            ...
            if len(copydirs) > 0:
                shutil.copy(os.path.join(cfg.scratch,pdfreport),os.path.join(cfg.scratch,deliverfolder))
                shutil.copy(cfg.readmefile,os.path.join(cfg.scratch,deliverfolder))
                myZipFile = zipfile.ZipFile(os.path.join(cfg.scratch,order_obj.number+"_US_Topo.zip"),"w")
                zipdir(os.path.join(cfg.scratch,deliverfolder),myZipFile)
                myZipFile.close()
            ...
            if needViewer == 'Y':
                ...
                dirs = filter(os.path.isdir, glob.glob(os.path.join(cfg.scratch,deliverfolder)+'\*_7.5_*'))
                if len(dirs) > 0:
