...
def myFirstPage(canvas, doc):
    canvas.saveState()
    canvas.drawImage(r"\\cabcvan1gis006\GISData\Topo_USA\python\mxd\ERIS_2018_ReportCover_Second Page_F.jpg",0,0, int(PAGE_WIDTH),int(PAGE_HEIGHT))
...
def createPDF(seriesText,diction,diction_s,outpdfname):
    ...
    #         mscale = 24000   #change on 11/3/2016, to fix all maps to the same scale
        ...
        if is_nova == 'Y':
            ...
            mxd = arcpy.mapping.MapDocument(mxdfile_terracon)
        else:
...
handler = logging.FileHandler(r'\\cabcvan1gis006\GISData\Topo_USA\python\log\USTopoSearch_Log.txt')
...
topouploadurl =  server_config["viewer_upload"] + r"/ErisInt/BIPublisherPortal_test/Viewer.svc/TopoUpload?ordernumber="#r"/ErisInt/BIPublisherPortal_prod/Viewer.svc/TopoUpload?ordernumber="

filepath = r'\\cabcvan1gis006\GISData\Topo_USA\python\mxd'
connectionPath = r'\\cabcvan1gis006\GISData\Topo_USA\python'
masterlyr = r'\\cabcvan1gis006\GISData\Topo_USA\masterfile\Cell_PolygonAll.shp'
csvfile_h = r'\\cabcvan1gis006\GISData\Topo_USA\masterfile\All_HTMC_all_all_gda_results.csv'
csvfile_c = r'\\cabcvan1gis006\GISData\Topo_USA\masterfile\All_USTopo_T_7.5_gda_results.csv'
...
mxdfile_terracon = os.path.join(filepath,'template_terracon.mxd')
mxdfile_terracon = r'\\cabcvan1gis006\GISData\Topo_USA\mxd\template_terracon.mxd'
...
# erislogopdf = r"\\cabcvan1gis006\GISData\Topo_USA\python\mxd\erislogo.pdf"
erislogojpg = os.path.join(filepath,"ERIS agency logo.jpg")
# emglogopdf = r"\\cabcvan1gis006\GISData\Topo_USA\python\mxd\emglogo.pdf"
...
try:
    ...
    #     cur.execute("select decode(c.company_id, 19803, 'Y', 'N') is_nova from orders o, customer c where o.customer_id = c.customer_id and o.order_id=" + str(OrderIDText))
    #     t = cur.fetchone()
    #     is_nova = t[0]
    ...
    logger.debug("#1")
    bufferDistance_e75 = '1 KILOMETERS'         # has to be not smaller than the search radius to void white page
